...
{
    readonly Magician_Player _player;
    readonly EntityAnimator _upperAniController;

    const float fFireTime = 0.7f;                              // 스킬 발사 타이밍(초)
    static readonly Vector3 vSpawnOffset = new Vector3(0f, 1f, 1f); // 발사 위치 오프셋(전방/높이, 로컬 기준)

    public MagicainUpperAttackState(Magician_Player player)
    {
        _player = player;
    ...
    void FireElectric()
    {
        ...
        Vector3 vDir = t.forward;
        Vector3 vSpawnPos = t.position + t.rotation * vSpawnOffset;

        GameManager.Instance.skillPool.UseSkill(
            SkillType.Electric, vSpawnPos, vDir, _player.OwnerClientId, _player.gameObject);
    }
}
